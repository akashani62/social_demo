...
describe("ModalController", () => {
  ...
  it("show prevents default on the triggering event", async () => {
  ...
  it("show opens the dialog and assigns the turbo frame src from the url value", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
        <button type="button" data-action="click->modal#show">Open</button>
        <dialog data-modal-target="dialog">
    ...
    expect(frame.src).toBe("/posts/new")
  })

  it("show does nothing when the dialog target is missing", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
        <button type="button" data-action="click->modal#show">Open</button>
        <turbo-frame id="new_post_modal" data-modal-target="frame"></turbo-frame>
  ...
  it("close closes an open dialog and clears the turbo frame", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
  ...
  it("closeOnBackdrop closes only when the dialog itself is the event target", async () => {
    const root = mountFixture(`
  ...
  it("afterSubmit closes the dialog when turbo:submit-end reports success", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
        <button type="button" data-action="click->modal#show">Open</button>
        <dialog data-modal-target="dialog">
  ...
  it("afterSubmit ignores failed submissions", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
        <button type="button" data-action="click->modal#show">Open</button>
        <dialog data-modal-target="dialog">
  ...
  it("closes on turbo:before-cache (Turbo snapshot) via connect hook", async () => {
    const root = mountFixture(`
      <div data-controller="modal" data-modal-url-value="/posts/new">
        <button type="button" data-action="click->modal#show">Open</button>
        <dialog data-modal-target="dialog">
  ...
  it("disconnect unregisters the turbo:before-cache listener", () => {
    const removeSpy = jest.spyOn(document, "removeEventListener")
    const beforeCache = () => {}

    ModalController.prototype.disconnect.call({ beforeCache })

    expect(removeSpy).toHaveBeenCalledWith("turbo:before-cache", beforeCache)
  })
})
